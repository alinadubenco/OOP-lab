...
public class Queue {
    ...
    public void push(Object object){
        ...
        {
            insert( object );  // because the code is the same a reasonable way is to create a function for a better understanding.

        }else if (maxsize==-1){  // the same but dont have a limit
            insert( object );  // the second use of function
        }
        else {
            System.out.println("Cannot add element.");  // this will happen when queue reached his maxsize;
        }
    }
    ...
    private void insert( Object object ) {
        if(count==0) {    // if this is the first node it will be created as the head and last element in the queue
            head=last= new Node( object );
            System.out.println("Element added: " + object);
            count++;
        }else {
            last.next = new Node( object );   // otherwise it will be created a node that will be linked to the previous node, and will become the last node
            System.out.println("Element added: " + object);
